...
const ROUTES = {
  NASHIK_SURAT: { label:'Nashik to Surat', sheet:'Nashik to Surat', code:'NS' },
  INDORE_SURAT: { label:'Indore to Surat', sheet:'Indore to Surat', code:'IS' }
};
...
function routeFromSheet_(sheetName) {
  if (sheetName === ROUTES.INDORE_SURAT.sheet) return 'INDORE_SURAT';
  return 'NASHIK_SURAT';
}
  ...
  try {
    ...
    const rows = b.passengers.map((pax, idx) => {
      ...
      return [
        ...
        route === 'INDORE_SURAT' ? b.boardingPoint : 'Nashik', 'Surat',
...
function validateGroupBooking_(b) {
  ...
  if (String(b.route) === 'INDORE_SURAT' && INDORE_NAGARS.indexOf(String(b.boardingPoint || '')) < 0) return 'BAD_BOARDING';
  const seatSet = {};
...
function setupSheets() {
  sheetForRoute_('NASHIK_SURAT');
  sheetForRoute_('INDORE_SURAT');
}
